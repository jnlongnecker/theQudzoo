...
function buildCustomSugarObject(displayName, innerClass, src = undefined, objectName = undefined) {
	...
	if (src !== undefined) {
		...
		let spacer = containerClass === 'icon-container' ? '&nbsp;&nbsp;' : '';
		imageMarkup = `<span class="${containerClass}">${spacer}<img class="inline-icon" src="!!!!" /></span>`;
	}
	let prefix = `<span class="injected"><span class="${innerClass}"><span class="nowrap">${imageMarkup}`;
